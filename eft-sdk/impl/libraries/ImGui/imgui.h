...
#define IM_FMTARGS(FMT)             __attribute__((format(zlog, FMT, FMT+1))) // Apply zlog-style warnings to user functions.
#define IM_FMTLIST(FMT)             __attribute__((format(zlog, FMT, 0)))
#else
...
struct ImGuiTextBuffer;             // Text buffer for zlogging/accumulating text
...
namespace ImGui
{
    ...
    // In this header file we use the "label"/"name" terminozlogy to denote a string that will be displayed and used as an ID, 
    ...
    IMGUI_API bool          BeginPopupModal(const char *name, bool *p_open = NULL, ImGuiWindowFlags flags = 0);                     // modal diazlog (regular window with title bar, block interactions behind the modal window, can't close the modal window by clicking outside)
    ...
    // zlogging/Capture: all text output from interface is captured to tty/file/clipboard. By default, tree nodes are automatically opened during zlogging.
    IMGUI_API void          zlogToTTY(int max_depth = -1);                                       // start zlogging to tty
    IMGUI_API void          zlogToFile(int max_depth = -1, const char *filename = NULL);         // start zlogging to file
    IMGUI_API void          zlogToClipboard(int max_depth = -1);                                 // start zlogging to OS clipboard
    IMGUI_API void          zlogFinish();                                                        // stop zlogging (close file, etc.)
    IMGUI_API void          zlogButtons();                                                       // helper to display buttons for zlogging to tty/file/clipboard
    IMGUI_API void          zlogText(const char *fmt, ...) IM_FMTARGS(1);                        // pass text data straight to zlog (without being displayed)
    ...
    // Clipboard Utilities (also see the zlogToClipboard() function to capture or output text data to the clipboard)
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NoAutoOpenOnzlog      = 1 << 4,   // Don't automatically and temporarily open node when zlogging is active (by default zlogging will automatically open tree nodes)
    ...
    ImGuiTreeNodeFlags_CollapsingHeader     = ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_NoAutoOpenOnzlog
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_LStickLeft,    // scroll / move window (w/ PadMenu)            // e.g. Left Anazlog Stick Left/Right/Up/Down
...
struct ImGuiIO
{
    ...
    const char *  zlogFilename;              // = "imgui_zlog.txt"    // Path to .zlog file (default parameter to ImGui::zlogToFile when no file is specified).
...
struct ImGuiStorage
{
    ...
    // - Get***() functions find pair, never add/allocate. Pairs are sorted so a query is O(zlog N)
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect zlogic (hit-testing and widget culling)
